...
double Integral(double a, double b, double s, double N, double truea, double trueb, double extraN)
{
	...
	if((a<1)||(b<1))
	{
		if(a<1)
		{
			...
			reachedhalf=false;
			Min=DeltaX0;
			DeltaX=1.00001*DeltaX0; //PROBLEMATIC POINT
			k=1;
			do{
				if(Min+DeltaX>0.5)
				{
...
double logProbBWS(double A1, double A0, int T1, int T0, double N, double s)
{
	...
	double Efluct, Efluctsqr, E, V, Enext, Vnext, EStar, VStar;
	double Aini, Anext;
	double a, b;
	double t;
	...
	//{
		...
		E=Aini;
		V=Aini*(1-Aini)/N;
		EStar=(E-P1ini)/(1-P1ini-P0ini);
		VStar=(V+E*E-P1ini)/(1-P1ini-P0ini)-EStar*EStar;
		a=alphastar(EStar,VStar);
		b=betastar(EStar,VStar);
		//cout << P0ini << "\t" << P1ini << "\t" << EStar << "\t" << VStar << "\t" << a << "\t" << b << endl;
...
{
	double P0ini, P1ini, P0next, P1next;
	double Efluct, Efluctsqr, E, Enext, V, Vnext, EStar,VStar;
	double Aini, Anext;
	double a, b;
	double t;
